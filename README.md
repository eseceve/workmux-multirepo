...
Inside tmux, wmm opens a builder window in the current session. Windows have change-specific names and metadata so multiple changes can share a session. Outside tmux, wmm creates a dedicated session and prints an attach command. Existing windows in another session must be closed before reopening the builder in the current session.
